...
/*
Set 28th 2020, leetcode challenge
...
public class subarrayProductLessThanK {
    ...
    //Raj implementation.Î
    /*
    public long numSubarrayProductLessThanK(int[] nums, int k) {

        long count =0;
        long output = nums.length ;

        //Set <Integer> inputset = new HashSet<Integer>() ;
       // inputset.addAll( ) ;
        Set<Integer> set = (Set<Integer>)Arrays.stream(nums).boxed().collect(Collectors.toSet());
        List<Integer> li = new ArrayList<>(set);

        if(li.size() == 1)
        {
            if( li.get(0)  < k) return  (output*(output+1)/2) ;
            else return 0 ;
        }
       /* for(int w:nums)
            inputset.add(w) ;
        ...
        if(inputset.size() ==1) {
                if(inputset.iterator().next() < k) return  (output*(output+1)/2) ;
                else return 0 ;
            }
        ...
        for(int i=0;i<nums.length;i++){
            for(int j=i;j<nums.length;j++) {
                ...
                for(int w=i;w<=j;w++) {
                    ...
                    if(prod >k) break ;
                    prod *= nums[w] ;
                    //System.out.println(" i: "+i+" j: " +j+ " w: "+w+ " prod: "+prod+ " count: "+count) ;
                }
               //System.out.println();
                if(prod<k) count += 1;
             }
...
*/
        ...
        for (int i=0;i<nums.length;i++)
        {
            ...
            if(nums[i]<k) {count += 1;}
            if (nums[i] * nums[i + 1] < k)  {count += 1;}

            long jprod = 1;
            for(int j=i+1;j<nums.length;j++) {
                jprod = jprod*nums[j] ;
            }
            if((nums[i]*jprod) < k) {count +=1 ;}
        }
        return count ;
...
*/
        ...
        int[] inputArray2 = new int[100000] ;
        for(int i=0;i<inputArray2.length;i++)
            inputArray2[i] = 1 ;

        // System.out.println("sub arrays   : "+ out.numSubarrayProductLessThanK(inputarray, 9931)) ;

      // System.out.println("sub arrays   : "+ out.numSubarrayProductLessThanK(inputArray2, 9)) ;

        //System.out.println("sub arrays   : "+ out.numSubarrayProductLessThanK(new int[] {10,2,2,5,4,4,4,3,7,7} , 289)) ;
        System.out.println("sub arrays   : "+ out.numSubarrayProductLessThanK(new int[] {1,2,3} , 2)) ;

        //System.out.println("sub arrays   : "+ out.numSubarrayProductLessThanK(new int[] {1,1,1} , 1)) ;
        //System.out.println("sub arrays   : "+ out.numSubarrayProductLessThanK(new int[] {10,9,10,4,3,8,3,3,6,2,10,10,9,3} , 19)) ;
        //System.out.println("sub arrays   : "+ out.numSubarrayProductLessThanK(new int[] {10,5,2,6} , 100)) ;
        //System.out.println("sub arrays   : "+ out.numSubarrayProductLessThanK(new int[] {10,9,10,4,3,8,3,3,6,2,10,10,9,3} , 19)) ;

       //System.out.println("sub arrays   : "+ out.numSubarrayProductLessThanK(new int[] {2,1,8,7,8,4,5,6,8,1,8,2,8,8} , 267)) ;
       //System.out.println("sub arrays   : "+ out.numSubarrayProductLessThanK(new int[] {1,2,3,4} , 20)) ;
        //System.out.println("sub arrays   : "+ out.numSubarrayProductLessThanK(new int[] {1,2,3,4} , 20)) ;

        /*
         Input: [2,1,8,7,8,4,5,6,8,1,8,2,8,8]
267
Output: 105
Expected: 42
         */
